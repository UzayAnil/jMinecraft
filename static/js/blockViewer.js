function BlockViewer(domElement, callbackL, callbackR) {
	var block_textures = [
		'anvil_base.png',
	...
	this.callbackR = callbackR;
	var texture_path = 'minecraft/textures/blocks/';

	function showBlockTextures(domElement) {
		var img, wrapper, name;
		for (var i = 0; i < block_textures.length; i++) {
			wrapper = document.createElement('div');
			wrapper.setAttribute('class', 'BlockImageWrapper');
			wrapper.setAttribute('data-texture-source', block_textures[i]);
			wrapper.addEventListener('click',
				(function(scope) {
					return function(e) {
						$('.clicked').removeClass('clicked');
						$(e.target).addClass('clicked');
						if (!callbackR || !e.shiftKey)
							callbackL(scope.getAttribute('data-texture-source'));
						else
							callbackR(scope.getAttribute('data-texture-source'));
					};
				})(wrapper), false
			);
			...
			img.setAttribute('src', texture_path + block_textures[i]);
			name.setAttribute('class', 'BlockTextureName');
			name.innerHTML = block_textures[i];
